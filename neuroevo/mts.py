...
and used by MOS
"""
from numpy import clip, zeros, copy
...
def _mtsls_improve_dim(function, chromosome, sol, best_fitness, i, check, SR):
    newsol = sol
    newsol[i] -= SR[i]
    newsol = check(newsol)
    fitness_newsol = function(chromosome, newsol)
    evals = 1
...
def mtsls(function, sol, fitness, lower, upper, maxevals, SR):
    ...
    current_best = EAresult(solution=sol.value, fitness=fitness, evaluations=0)
    totalevals = 0
    ...
    if totalevals < maxevals:
        ...
        for i in dim_sorted:
            result = _mtsls_improve_dim(function, sol, current_best.solution, current_best.fitness, i, check, SR)
