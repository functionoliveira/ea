...
from crossover import classicalBinFunction, SimpleCrossover
from ea_parameters import get_parameter, get_value, parameter_result
import types
import six
...
def DE(fun,
       ...
       CR=0.9,
       popsize=60,
       crossoverFunction=classicalBinFunction,
       population=None,
    ...
    if isinstance(crossoverFunction, types.FunctionType):
        crossover = SimpleCrossover(crossoverFunction)
    else:
    ...
    for numrun in range(1, run + 1):
        ...
        for i in range(sizePopulation):
            ...
            if bestSolutionFitness > populationFitness[i]:
                ...
                msg = "Best solution Find: %f at %d" % (bestSolutionFitness,
                                                        currentEval)
                dprint(msg, debug)
        ...
        while not shouldEnd(currentEval, maxEval, bestSolutionFitness,
            ...
            for i in range(sizePopulation):
                ...
                if populationFitness[i] < bestSolutionFitness:
                    ...
                    dprint("Best solution Find: %f at %d" %
        ...
        msg = "The best solution is: %f in %d evaluation %s" % (
